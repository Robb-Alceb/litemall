...
/**
 * @author ：stephen
 * @date ：Created in 2020/6/9 11:27
 * @description：TODO
 */
@Service
public class WebCartService {
    private static final Log log = LogFactory.getLog(WebCartService.class);
            ...
            if(cartDto.getCartGoodsAccessoryList() != null && cartDto.getCartGoodsAccessoryList().size() > 0){
                for(LitemallCartGoodsAccessory item : cartDto.getCartGoodsAccessoryList()){
                    ...
                    if(merchandise == null || merchandise.getNumber() < item.getNumber() * number){
                        return ResponseUtil.fail(WebResponseEnum.ACCESSORY_ENOUGH);
                    }
                    //设置辅料项
